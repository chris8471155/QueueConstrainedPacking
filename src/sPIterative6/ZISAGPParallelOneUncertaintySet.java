																...
																for(int v=0;v<useTriangularDistributionGeneToMutate.length;v++){
																	for(int w=0;w<levelOfVehicleIntersectionMixRelations.length;w++){
																		for(int x=0;x<yardPolicyChoices.length;x++){
																			...
																			callMain(expNameString, ferryShapes[i], numbersOfLanes[j], classInstanceNumbers[k], objectiveFunctions[l], breakTiesLestFulls[m], currentExperimentNumber, numberOfArrivalScenarios[n], yardPolicyChoices[x], moduloScenarios[o], yardPolicyTypes[p], comittedVMixQuantileIndices[n][q], intersectionObjectiveWeights[r], iterationsSubIterations[s], T0s[t], cumuNighbourDistributions[u], visualiseBestSolutions, initialArrivalScenarioSeeds[nn], useTriangularDistributionGeneToMutate[v], resultsOutput, commitIntersectionVMix, arrivingVehicles, performLSFinishingTouch);																		
	...
	static void callMain(String expNameString, String ferryShape, int numberOfYardLanes, int[] classAndInstance, int objectiveFunctionIndex, boolean breakTiesWithLeastFullLane, int ExperimentNumber, int numberOfArrivalScenarios, boolean useRuleBasedYardPolicy, boolean moduloScenario, int yardPolicyType, int comitVMixPercentileIndex, double intersectionObjectiveWeight, int[] iterationsSubIterations, double T0, double[] cumuNighbourDistribution, boolean visualiseBestSolutions, int initialArrivalScenarioSeed, boolean useTriangularDistributionGeneToMutate, PrintWriter resultsOutput, boolean commitIntersectionVMix, int[] arrivingVehicles, boolean performLSFinishingTouch) throws Exception{
		...
		//for(int ii=0;ii<testInstanceClassSet.length;ii++){
			//for(int jj=0;jj<instancesPerInstanceClass[ii].length;jj++){
				...
				GeneralContainer.numberOfOrientations=numberOfOrientations;
				GeneralContainer.numberOfVehicleSelectionQuantiles=numberOfVehicleSelectionQuantiles;
...
*/				Solution.alphabetLength=4*rectangleDistribution[0].length;//alphabetLength;
				...
				boolean justGenerateInstanceFiles=false;//true;//
				if(justGenerateInstanceFiles) {
					//////////////////////////////
					///////////////////////////////
					String instanceDetailsFileName="InstanceDetails".concat("Class").concat(String.valueOf(testInstanceClass-4)).concat("Instance").concat(String.valueOf(instanceNumber)).concat(".txt");
					PrintWriter instanceW=new PrintWriter(new FileOutputStream(instanceDetailsFileName));
					double ferryLength=bestSolContainer[0].top-bestSolContainer[0].bottom;
					double ferryWidth=bestSolContainer[0].right-bestSolContainer[0].left;
					instanceW.println(ferryLength+","+ferryWidth);
					for(int v=0;v<YA.vCountByType.length;v++) {
						if(YA.vCountByType[v]>0) {
							instanceW.println(YA.rectTypes[v].l+","+YA.rectTypes[v].w+","+YA.vCountByType[v]);
						}
					...
					String instanceClass1PrefixBestEver="bestSolutionsGP/TestInstanceClass".concat(String.valueOf(testInstanceClass)).concat(ferryShape).concat("BestUtilisation").concat(String.valueOf(instanceNumber)).concat("VehicleTypes.txt");
					...
					if(maxPackedArea>bestPackedAreaPreviously && visualiseBestSolutions){
						...
						String bestSolutionFileName="bestSolutionsGP/TestInstanceClass".concat(String.valueOf(testInstanceClass)).concat(ferryShape).concat("BestSolution").concat(String.valueOf(instanceNumber)).concat("VehicleTypes.txt");
						...
						for(int i=0;i<bestSolContainer[bestSolutionIndex].parkedRectangles.size();i++){
							Rectangle currentRectangle=bestSolContainer[bestSolutionIndex].parkedRectangles.get(i);
							bestSolW.print("("+nonZeroVTypeIndex[currentRectangle.type]+","+currentRectangle.parkedPosition[0]+","+currentRectangle.parkedPosition[1]+"),");
						}
						...
						String instanceClass1PrefixImage="bestSolutionsGP/TestInstanceClass".concat(String.valueOf(testInstanceClass)).concat(ferryShape).concat("BestSolutionImage").concat(String.valueOf(instanceNumber)).concat("VehicleTypes");
					...
					*/
					...
					//bestSolContainer.implementSolutionSimplifiedStrips(randomArrivalScenarioSeeds);
					...
					packingSolutionW.println(Solution.alphabetLength+","+Solution.maxSolutionLength+","+Solution.numberOfQuantiles+","+Solution.numberOfStripTypes+","+GeneralContainer.numberOfOrientations+","+GeneralContainer.numberOfVehicleSelectionQuantiles+","+objectiveFunctionIndex+","+onlineRecourseAlgorithmName+","+tt);
					/*for(int i=0;i<GeneralContainer.setOfQuantiles.length;i++){
						packingSolutionW.print(GeneralContainer.setOfQuantiles[i]+",");
					}
	...
	GeneralContainer[] solveRecourseProblem(int[] arrivalScenarioRandomSeeds, Yard YA, Random randNumGen, String ferryShape, double totalRectangleArea, int objectiveFunctionIndex, boolean breakTiesWithLeastFullLane, GeneralContainer[] bestSolContainer, boolean printProgress, boolean bestSolContainerContainsInitialSolution, boolean onlineRecourseProblem, double claimedUtil, boolean recourseSolverOn, boolean useRuleBasedYardPolicy, boolean moduloScenario, int yardPolicyType, int subsetSize, double intersectionObjectiveWeight, int[] iterationsSubIterations, boolean printProgressOnConsole, double T0, double[] cumuNighbourDistributions, double minWidth, double minLength, boolean visualiseBestSolutions, boolean useTriangularDistributionGeneToMutate, PrintWriter resultsOutput, boolean commitIntersectionVMix, boolean performFinishingTouchLocalSearch) throws Exception{
		...
		GeneralContainer[] currentSol=new GeneralContainer[numberOfArrivalScenarios];//(YA, randNumGen, false, ferryShape, 0, totalRectangleArea, arrivalScenarioRandomSeeds.length);//each packing solution is evaluated in each arrival scenario
		GeneralContainer[] neighbourSol=new GeneralContainer[numberOfArrivalScenarios];//(YA, randNumGen, false, ferryShape, 1, totalRectangleArea, arrivalScenarioRandomSeeds.length);
		//GeneralContainer[] bestPackSols=new GeneralContainer[numberOfArrivalScenarios];//packing iterations are like subproblems. This array stores the best packing solutions for the incumbent yard policy 
		...
		for(int s=0;s<numberOfArrivalScenarios;s++){
			currentSol[s]=new GeneralContainer(YA, randNumGen, false, ferryShape, s, totalRectangleArea, numberOfArrivalScenarios, minWidth, minLength);//each packing solution is evaluated in each arrival scenario
			neighbourSol[s]=new GeneralContainer(YA, randNumGen, false, ferryShape, s+numberOfArrivalScenarios, totalRectangleArea, numberOfArrivalScenarios, minWidth, minLength);
			//bestPackSols[s]=new GeneralContainer(YA, randNumGen, false, ferryShape, s+2*numberOfArrivalScenarios, totalRectangleArea, numberOfArrivalScenarios);
		...
		for(int s=0;s<numberOfArrivalScenarios;s++){
			//currentSol.implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null);
			currentSol[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);
			...
			ScenarioSolution newScenarioSolution2=new ScenarioSolution(currentSol[s].sol, currentSol[s].YA.sol, currentSol[s].vMixes, s);//currentSol[s]
		...
		for(int s=0;s<numberOfArrivalScenarios;s++){
			...
			bestSolContainer[s].implementSolutionSimplifiedStrips(arrivalScenarioArrivalScenarioSeeds[s], 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);//-bestObj[s]+0.000000000000001
			//boolean useValues2=true;//arrivalScenarioArrivalScenarioSeeds[s]
			//bestSolContainer[s].implementSolutionQueueConstrainedOptimallyFillBins(arrivalScenarioArrivalScenarioSeeds2[s], 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, bestSolContainer[s].totalArea, false, randNumGen, useValues2);
		...
		for(int s=0;s<numberOfArrivalScenarios;s++){// && !maximumUtilEstablished
			...
			bestSolContainer[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);//-bestObj[s]+0.000000000000001
			//now that values2 are filled in, 
			//bestSolContainer[s].implementSolutionQueueConstrainedOptimallyFillBins(arrivalScenarioRandomSeeds[1], 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, bestSolContainer[s].totalArea, false, randNumGen, true);
			...
			ScenarioSolution newScenarioSolution=new ScenarioSolution(bestSolContainer[s].sol, bestSolContainer[s].YA.sol, bestSolContainer[s].vMixes, s);//bestSolContainer[s]
			bestPackSolScenSolArray[s]=newScenarioSolution;
			SSL.addScenarioSolutionToNonDominatedSolutionsLinkList(newScenarioSolution);
		...
		if(SSL.maxIntersectionArea>bestOverallComittedUtilisation){
			...
			if(assignBestSolutionsToScenarioContainers){
				...
				for(int s=0;s<numberOfArrivalScenarios;s++){
					...
					bestSolContainer[s].implementSolutionSimplifiedStrips(arrivalScenarioArrivalScenarioSeeds[s], 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);
				}
				/*if(!visualiseBestSolutions){
					for(int s=0;s<numberOfArrivalScenarios;s++){
						bestSolContainer[s].setVisible(false);
			...
			for(int subIteration=0;subIteration<iterationsPerBigIteration[yardIteration];subIteration++){// && (!onlineRecourseProblem || (onlineRecourseProblem && claimedUtil>bestUtil))
				...
				if(yardIteration==0){
					for(int s=0;s<numberOfArrivalScenarios;s++){
						...
						neighbourSol[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, yardIteration, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, generateARandomReasonablePackingSolution, randNumGen);
						...
						if(acceptNewSolution){
							...
							if(obj>bestObj[s]){
								...
								//bestPackSols[s].implementSolutionSimplifiedStrips(arrivalScenarioArrivalScenarioSeeds[s], 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);
							}
						}
						...
						if(neighbourSol[s].intersectionObjVal>bestOverallComittedUtilisation){
							...
							/*if(printProgressOnConsole){
								if(counter>-1){
									...
									bestSolContainer[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, yardIteration, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null);//generateARandomReasonablePackingSolution
									for(int ss=0;ss<numberOfArrivalScenarios;ss++){
										bestSolContainer[s].implementSolutionSimplifiedStrips(arrivalScenarioArrivalScenarioSeeds[ss], 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null);
										//System.out.print(bestSolContainer[s].objValPerArrivalScenario[ss]+",");
									}
									//System.out.println();
							...
							if(assignBestSolutionsToScenarioContainers){//sometimes its good to forget
								...
								for(int ss=0;ss<numberOfArrivalScenarios;ss++){
									...
									bestSolContainer[ss].implementSolutionSimplifiedStrips(arrivalScenarioArrivalScenarioSeeds[ss], 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);
									if(printProgressOnConsole){
								...
								//if(printProgressOnConsole){
									...
									for(int sss=0;sss<numberOfArrivalScenarios;sss++){
										bestSolContainer[sss].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);
										for(int ss=0;ss<numberOfArrivalScenarios;ss++){
					...
					for(int s=0;s<numberOfArrivalScenarios;s++){// && !maximumUtilEstablished
						...
						currentSol[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);//-bestObj[s]+0.000000000000001
					...
					if(obj>bestOverallComittedUtilisation){
						...
						if(assignBestSolutionsToScenarioContainers){
							...
							for(int s=0;s<numberOfArrivalScenarios;s++){//
								...
								bestSolContainer[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);
								for(int ss=0;ss<numberOfArrivalScenarios;ss++){
									System.out.print(bestSolContainer[s].objValPerArrivalScenario[ss]+",");
								}
								System.out.println();
					...
					for(int s=0;s<numberOfArrivalScenarios;s++){// && !maximumUtilEstablished
						...
						neighbourSol[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, 1, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);//-bestObj[s]+0.000000000000001
						//now that values2 are filled in, 
								...
								for(int e=0;e<activeLengthOfSolution;e++) {
									for(int a=0;a<alphabetLength;a++) {
										...
										if(neighbourSol[s].YA.sol.mutationTypeNeighbour(e, a)) {
											//implementa solution   arrivalScenarioArrivalScenarioSeeds[s]
											neighbourSol[s].implementSolutionSimplifiedStrips(arrivalScenarioRandomSeeds, yardIteration, breakTiesWithLeastFullLane, useRuleBasedYardPolicy, moduloScenario, yardPolicyType, null, false, randNumGen);
